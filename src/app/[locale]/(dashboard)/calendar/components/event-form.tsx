...
const eventTypes = [
  { value: "meeting", label: "Meeting", color: "bg-[#E8401A]" },
  { value: "event", label: "Event", color: "bg-neutral-800" },
  { value: "personal", label: "Personal", color: "bg-neutral-500" },
  { value: "task", label: "Task", color: "bg-[#E8401A]" },
  { value: "reminder", label: "Reminder", color: "bg-neutral-400" }
]
...
export function EventForm({ event, open, onOpenChange, onSave, onDelete }: EventFormProps) {
  ...
  const handleSave = () => {
    const eventData: Partial<CalendarEvent> = {
      ...formData,
      id: event?.id,
      color: eventTypes.find(t => t.value === formData.type)?.color || "bg-[#E8401A]"
    }
    onSave(eventData)
    onOpenChange(false)
  }
