...
export function SalesChart() {
  ...
  const filtered =
    timeRange === "3m" ? allData.slice(-3) : timeRange === "6m" ? allData.slice(-6) : allData
    ...
    <Card className="cursor-pointer">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Select value={timeRange} onValueChange={setTimeRange}>
          ...
          <SelectContent>
            <SelectItem value="3m" className="cursor-pointer">{t("last3m")}</SelectItem>
            <SelectItem value="6m" className="cursor-pointer">{t("last6m")}</SelectItem>
            <SelectItem value="12m" className="cursor-pointer">{t("last12m")}</SelectItem>
          </SelectContent>
        </Select>
      </CardHeader>
